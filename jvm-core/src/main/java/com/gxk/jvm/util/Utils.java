...
public abstract class Utils {
    ...
    public static String genNativeMethodKey(Method method) {
        return genNativeMethodKey(method.clazz.getName(), method.getName(), method.getDescriptor());
    }
    ...
    public static String obj2Str(Instance name) {
        if (!name.clazz.getName().equals("java/lang/String")) {
    ...
    public static Instance str2Obj(String str, ClassLoader classLoader) {
        Class klass = Heap.INSTANCE.findClass("java/lang/String");
    ...
    public static String classpath(String classpath) {
        ...
        if (miniJvmHome == null) {
            ...
            } else if (userDir.contains("mini-jvm")) {
                miniJvmHome = userDir;
            }
        }
        if (miniJvmHome == null) {
    ...
    public static void doReturn(int slotSize) {
        ...
        // 解释器同步执行方法的结束条件
        if (old.getStat() == Const.FAKE_FRAME) {
            old.setStat(Const.FAKE_FRAME_END);
        }
    ...
    public static void clinit(Class clazz) {
        // 类在初始化中，或已完成初始化，直接返回
        if (clazz.getStat() >= Const.CLASS_INITING) {
            return;
        }
...
//    // 存在没有类初始化的情况
        ...
        Interpreter.INSTANCE.execute(newFrame);
        clazz.setStat(Const.CLASS_INITED);
    }
    ...
    public static void invokeMethod(Method method) {
        ...
        if (Utils.isNative(method.getAccessFlags())) {
